...
Delivery status must remain explicit:

- Ford evidence describes a production manufacturing system.
...
Adaptive Focus ranks a unified evidence-entity list containing public project IDs followed by professional-experience IDs. The deterministic matcher may place either entity type in primary, supporting, or adjacent proof.
...
Model evaluation and operational calibration are separate capabilities. Model evaluation covers direct assessment of AI responses, reasoning quality, correctness, and failure modes. Evaluation and calibration covers operational review systems, calibration practices, and review consistency.

## Route Policy

Former proprietary case-study routes permanently redirect to `/about#professional-experience` before public project lookup. Deleted media paths are not redirected and must return `404`. Retired projects are excluded from navigation, metadata, media configuration, and generated public project collections.

Deleting files from the current tree does not erase prior Git commits, forks, deployment artifacts, or external caches. Historical remediation requires a separate decision.
...
Before converting professional experience into a public case study:

1. Obtain explicit approval for the exact copy, media, project name, and metrics.
2. Add only approved assets to a new public project directory.
3. Add a canonical public project record and curation entry.
4. Move or add evidence statements that cite the new public route.
5. Add route, media, privacy, and browser tests.
6. Confirm the professional summary does not preserve conflicting or more detailed hidden data.
