...
describe("server-first homepage interactions", () => {
  ...
  const analyticsMarkupSource = readSource("components/static-route-analytics.tsx")
  ...
  it("keeps the lightweight analytics bridge consent-gated and allowlisted", () => {
    ...
    expect(analyticsMarkupSource).toContain("PORTFOLIO_ANALYTICS_PROPERTY_ALLOWLIST")
    expect(analyticsMarkupSource).toContain("data-property-allowlist=")
    expect(analyticsMarkupSource).toContain("data-page-path={pathname}")
